...
// const taskPage: PaginatedResponse<TimedTask> = {
...
function printID(id: ID): void {
  if (typeof id === 'string') {
    ...
    console.log("Number ID doubled:", id * 2);
  }
}
...
function sendMessage(c: Contact) {
  if (c.type === 'email') {
    console.log(`Sending email to ${c.emailAddress}`);
  } else {
    console.log(`Calling phone number ${c.phoneNumber}`);
  }
}

sendMessage({ type: "email", emailAddress: "mike@example.com" });
sendMessage({ type: "phone", phoneNumber: "555-1234" });